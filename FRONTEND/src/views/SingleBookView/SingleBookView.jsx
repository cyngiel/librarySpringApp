...
export const SingleBookView = () => {
  const dispatch = useDispatch();
	const { id } = useParams();
	const [ { title, author, category, date, description, available_items, items } ] = booksData.filter((book) => book.id === id);
  
	const handleAddBook = () => dispatch(addBook(id))

	return (
		<article className={styles.wrapper}>
			<img src='https://picsum.photos/300/300' alt='book' className={styles.picture}/>
